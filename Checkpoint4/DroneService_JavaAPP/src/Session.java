...
public class Session {
    ...
    public void createElement(Scanner input) {
        ...
        Element newElement = new Element(serial, name, model, manufacturer,
                warranty, year);
        Element_Record.put(serial, newElement);
        System.out.println("New Element has been created: ");
        System.out.println(newElement.showSerialNum() + " "
                + newElement.showName() + "\n");
    }
    ...
    public void createPerson(Scanner input) {
        ...
        Person newPerson = new Person(phone, firstName, lastName, address,
                email);
        Person_Record.put(phone, newPerson);
        System.out.println("New Person has been created: ");
        System.out.println(
                newPerson.showPhone() + " " + newPerson.showName() + "\n");
    ...
    public void createWarehouse(Scanner input) {
        ...
        Warehouse newWarehouse = new Warehouse(storageCapacity, droneCapacity,
                address, phone, manager, city);
        Warehouse_Record.put(address, newWarehouse);
        System.out.println("New Warehouse has been created: ");
        System.out.println(
                newWarehouse.showLocation() + " " + newWarehouse.showAddress()
                        + " " + newWarehouse.showPhoneNum() + "\n");
    ...
    public void displaySearch(Scanner input) {
        ...
        while (another == true) {
            ...
            switch (selection) {
                case "a":
                    System.out.println("\n* ELEMENTS *");
                    for (Map.Entry<String, Element> option : this.Element_Record
                            .entrySet()) {
                        Element current = option.getValue();
                        String serial = option.getKey();
                        System.out.println(
                                "[" + serial + "] " + current.showName());
                    }
                    this.editElement(input);
                    break;
                case "b":
                    System.out.println("\n* PERSONS *");
                    for (Map.Entry<String, Person> option : this.Person_Record
                            .entrySet()) {
                        Person current = option.getValue();
                        String phone = option.getKey();
                        System.out.println(
                                "[" + phone + "] " + current.showName());
                    }
                    this.editPerson(input);
                    break;
                case "c":
                    System.out.println("\n* WAREHOUSES *");
                    for (Map.Entry<String, Warehouse> option : this.Warehouse_Record
                            .entrySet()) {
                        Warehouse current = option.getValue();
                        String address = option.getKey();
                        System.out.println(
                                "[" + address + "] " + current.showLocation());
                    }
    ...
    public void editWarehouse(Scanner input) {
        ...
        do {
            System.out
                    .println("Enter 's' to select a record or 'd' to Go Back");
            String selection = input.nextLine();
            switch (selection) {
                case "s":
                    ...
                    while (!validWarehouse) {
                        System.out
                                .println("Enter the Address of the Warehouse");
                        address = input.nextLine();
                        if (Warehouse_Record.containsKey(address)) {
                            validWarehouse = true;
                        } else {
                            System.out.println(
                                    "INVALID WAREHOUSE ADDRESS: ** TRY AGAIN **");
                        }
                    }
                    ...
                    do {
                        validAnswer = true;
                        System.out.println(
                                "Enter 'e' to edit the Warehouse at " + address
                                        + " or 'd' to delete the Warehouse at "
                                        + address);
                        ...
                        if (answer.equals("e")) {
                            ...
                            do {
                                ...
                                switch (attribute) {
                                    ...
                                    case "3":
                                        System.out.println(
                                                "Enter New Drone Capacity:");
                                        String drone = input.nextLine();
                                        warehouse.droneCapacity = drone;
                                        System.out.println(
                                                "New Drone Capacity for "
                                                        + warehouse.address
                                                        + " is "
                                                        + warehouse.droneCapacity);
                                    ...
                                    case "4":
                                        ...
                                        System.out.println("New Manager for "
                                                + warehouse.address + " is "
                                                + warehouse.manager);
                                        break;
                                    default:
                                        System.out.println(
                                                "INVALID INPUT: PLEASE ENTER INTEGER BETWEEN 1-4");
            ...
            switch (selection) {
                case "s":
                    ...
                    do {
                        validAnswer = true;
                        System.out.println(
                                "Enter 'e' to edit the Person with number "
                                        + phone
                                        + " or 'd' to delete the Person with number "
                                        + phone);
                        String answer = input.nextLine();
                        if (answer.equals("e")) {
                            ...
                            do {
                                ...
                                switch (attribute) {
                                    case "1":
                                        ...
                                        person.fName = fName;
                                        System.out.println("New First Name for "
                                                + person.phone + " is "
                                                + person.fName);
                                        break;
                                    case "2":
                                    ...
                                    case "3":
                                        ...
                                        System.out.println("New Address for "
                                                + person.phone + " is "
                                                + person.address);
                                    ...
                                    case "4":
                                        ...
                                        String email = input.nextLine();
                                        person.email = email;
                                        System.out.println("New Email for "
                                                + person.phone + " is "
                                                + person.email);
                            ...
                            System.out.println("Person with Number " + phone
                                    + " has been removed");
                        } else {
                            System.out.println("Invalid entry, try again");
                            validAnswer = false;
        ...
        do {
            ...
            switch (selection) {
                case "s":
                    ...
                    while (!validElement) {
                        System.out.println(
                                "Enter the Serial Number of the Element");
                    ...
                    do {
                        validAnswer = true;
                        System.out.println(
                                "Enter 'e' to edit the Element with serial "
                                        + serial
                                        + " or 'd' to delete the Element with serial "
                                        + serial);
                        String answer = input.nextLine();
                        if (answer.equals("e")) {
                            ...
                            do {
                                ...
                                switch (attribute) {
                                    ...
                                    case "3":
                                        System.out.println(
                                                "Enter New Warranty Expiration:");
                                        String warranty = input.nextLine();
                                        element.warrantyExpiration = warranty;
                                        System.out.println(
                                                "New Warranty Expiration for "
                                                        + element.serialNum
                                                        + " is "
                                                        + element.warrantyExpiration);
                                        break;
                                    default:
                                        System.out.println(
                                                "INVALID INPUT: PLEASE ENTER INTEGER BETWEEN 1-4");
                            ...
                            System.out.println("Element with Serial Number "
                                    + serial + " has been removed");
                        } else {
                            System.out.println("Invalid entry, try again");
                            validAnswer = false;
                        }
